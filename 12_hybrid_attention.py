#!/usr/bin/env python3
"""
12_hybrid_attention.py
======================
Dual-Attention Hybrid Model.
Extracts unpooled features from ESM-2 and DNABERT, applies separate Attention mechanisms
to both, concatenates the context vectors, and classifies.
"""
...
class HybridAttentionClassifier(nn.Module):
    def __init__(self, esm_hidden=320, dnabert_hidden=768):
        ...
        hybrid_hidden = esm_hidden + dnabert_hidden
        self.classifier = nn.Sequential(
            nn.Linear(hybrid_hidden, 128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 1)
        )
    ...
    def forward(self, x_esm, x_dnabert):
        # x_esm: (num_orfs, 320), x_dnabert: (num_windows, 768)
        context_esm = self.esm_attn(x_esm)
        context_dnabert = self.dnabert_attn(x_dnabert)
        
        context_hybrid = torch.cat([context_esm, context_dnabert], dim=1) # (1, 1088)
...
def main():
    ...
    X_esm_raw = get_unpooled_esm_features(all_seqs, device, desc="All Sequences")
    X_dnabert_raw = get_unpooled_dnabert_features(all_seqs, device, max_windows=args.max_windows, desc="All Sequences")

    # We need to split indices so both modalities match
    ...
    model = HybridAttentionClassifier(esm_hidden=320, dnabert_hidden=768).to(device)
    criterion = nn.BCEWithLogitsLoss()
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)

    print("\nTraining Hybrid Attention Model...")
    model.train()
    ...
    print("\n=== Hybrid Attention Prototype Results ===")
